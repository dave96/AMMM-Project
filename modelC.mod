 ...
 *********************************************/
 ...
 dvar boolean vo[p in P][o in O];
 
 // Constant. model A is a feasibility-only model.
 minimize sum(p in P, o in O) vo[p][o];
 ...
 subject to {
 	...
 	// Check the "window"
 	forall(p in P, o in O)
 	  vo[p][o] == (sum(pk in p..(p + k[o] - 1) : (p + k[o] - 1 <= nPositions)) po[pk][o] >= (m[o] + 1));
 }
 ...
 execute {
 	...
 	write("TOTAL VIOLATIONS: " + totalViolations);
 }